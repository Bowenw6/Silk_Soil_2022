...
ASV_filtered_fasta_16S<-re_Fasta_16S[match(row.names(ASV_filtered_16S), row.names(re_Fasta_16S)),]

print(row.names(ASV_filtered_fasta_16S)==row.names(ASV_filtered_16S))

ASV_filtered_fasta_16S$ASVs_names<- gsub("ASV",">ASV",ASV_filtered_fasta_16S$ASVs_names)

write.table(ASV_filtered_fasta_16S, "ASV_filtered_fasta_16S.fasta",sep =",", row.names =FALSE, col.names =FALSE, quote =FALSE)

filtered_fasta_16S<-readLines("ASV_filtered_fasta_16S.fasta",encoding = "UTF-8")
...
paired_beta_nti_silk <- beta_nti(otu_niche = ASV_Silk_16S, otu_tree = tree_16S,1000,8)

write.table(paired_beta_nti_silk,"paired_beta_nti_silk.txt",quote = FALSE,
            sep = "\t",row.names = TRUE,col.names = TRUE)

paired_beta_nti_soil <- beta_nti(otu_niche = ASV_Soil_16S, otu_tree = tree_16S,1000,8)
...
paired_raup_crick_silk<-raup_crick_abundance(ASV_Silk_16S,plot_names_in_col1 = FALSE,threads = 8)
...
paired_raup_crick_soil<-raup_crick_abundance(ASV_Soil_16S,plot_names_in_col1 = FALSE,threads = 8)
