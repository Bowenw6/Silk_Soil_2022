...
library(edgeR)
library(ggpubr)
otu<-read.csv("16S_ASV.csv",row.names = 1)
design<-read.table("design.txt",header = TRUE)
## compare Silk and Soil 
#DGElist
...
#import ASVs annotation
Taxonomy_16S<-read.table("16S_Taxonomy.txt", sep="\t", header = TRUE, row.names = 1)

reorderd_tt_enrich<- tt_enrich[match(row.names(Taxonomy_16S),row.names(tt_enrich)),]

print(row.names(diff_result_16S)==row.names(Taxonomy_16S))

diff_result_16S<- cbind(reorderd_tt_enrich,Taxonomy_16S)
diff_result_16S$threshold<- ifelse(diff_result_16S$logFC>=0&diff_result_16S$FDR<=0.05,"Enriched on silk",
                                   ifelse(diff_result_16S$logFC<0&diff_result_16S$FDR<=0.05,
                                          "Depleted on silk","Not significant"))
#export Table S5
write.csv(diff_result_16S,'diff_result_16S.csv', quote = FALSE)
...
#identify Enriched and Depleted ASVs
...
#Radar plot
Radar_plot$Max<- rep(18,22)
Radar_plot$Min<- rep(0,22)
...
# Add a legend
legend(x=0.5, y=-0.5, legend = rownames(Radar_plot[-c(2,1),]), bty = "n", pch=20 , col=c("#55AFFF", "#F4B800") , text.col = "black", cex=1, pt.cex=3) #pch → marker shape
...
#Manhattan plot Figure 5D
diff_result_16S<-read.csv("diff_result_16S.csv", row.names = 1,na.strings = "") 
Manhattan_data<- diff_result_16S[,c("logCPM","FDR","threshold","Phylum")]
...
Man_top_phylum=c("Proteobacteria","Actinobacteria","Patescibacteria","Bacteroidetes","Cyanobacteria","Planctomycetes","Firmicutes","Acidobacteria","Nitrospirae","Chlamydiae","Dependentiae","Verrucomicrobia","Rokubacteria", "BRC1", "Armatimonadetes","Chloroflexi","Dadabacteria","Entotheonellaeota","Gemmatimonadetes", "Hydrogenedentes","Latescibacteria", "Unknown")
...
#save image as pdf 7 inches*14 inches
