...
#ITS_Silk DOC plot
library(DOC)
Silk_ITS_DOC_results <- DOC(ASV_Silk_ITS, R = 1000)
#The P-value was calculated as the fraction of bootstrap runs (n_boot=1000) resulting with non-negative slopes.
silk_p_value<-(sum(Silk_ITS_DOC_results$LME$Slope >= 0) + 1) / (length(Silk_ITS_DOC_results$LME$Slope) + 1)
p1<-plot(Silk_ITS_DOC_results) + theme_bw() +geom_text(aes(x = 0.45,y = 0.95,
                                          label = paste("Silk-Fungus", '\n', "P=", round(silk_p_value, 4)))) 
p1
...
#ITS_Soil DOC plot
library(DOC)
Soil_ITS_DOC_results <- DOC(ASV_Soil_ITS, R = 1000)
#The P-value was calculated as the fraction of bootstrap runs (n_boot=1000) resulting with non-negative slopes.
soil_p_value<-(sum(Soil_ITS_DOC_results$LME$Slope >= 0) + 1) / (length(Soil_ITS_DOC_results$LME$Slope) + 1)
p2<-plot(Soil_ITS_DOC_results) + theme_bw() +geom_text(aes(x = 0.85,y = 0.95,
                                                       label = paste("Soil-Fungus", '\n', "P=", round(soil_p_value, 4)))) 
p2
#save as pdf 4inches*4inches
