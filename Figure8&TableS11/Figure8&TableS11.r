...
All_16S <- read.table(file = "all_16S.txt",header = T, row.names = 1, sep = "\t")





ASV_filtered_16S<-All_16S[which(rowSums(All_16S)>=30),]
...
ASV_filtered_16S$Group<-if_else(grepl("*Si", rownames(ASV_filtered_16S)), "Silk","Soil")

ASV_filtered_16S$Group<-as.factor(ASV_filtered_16S$Group)



set.seed(666)

treat_rf <- randomForest(as.factor(Group) ~ ., data= ASV_filtered_16S,importance=TRUE,proximity=TRUE)
treat_rf
plot(treat_rf, main = "RandomForest origin")
#save image 5inches*4inches

set.seed(666)
treat_perm <- rf.significance(treat_rf, ASV_filtered_16S, nperm=99, ntree=500)
treat_perm



Group_rfP<- rfPermute(Group ~ ., data = ASV_filtered_16S, ntree = 500,
...
#Define MeanDecreaseAccuracy.pval<0.01 and MeanDecreaseAccuracy>=2% as Top features
...
#Top_features
write.csv(Top_features, file="Top_features_16S.csv", quote = FALSE)


ASV_16S_Taxonomy<-read.table(file = "ASV_16S_Taxonomy.txt", sep = "\t", row.names = 1, header = TRUE)

ASV_16S_Taxonomy_filtered<-ASV_16S_Taxonomy[match(row.names(Top_features), row.names(ASV_16S_Taxonomy)),]

ASV_16S_Taxonomy_filtered[ASV_16S_Taxonomy_filtered==""]<- "Unknown"

print(row.names(ASV_16S_Taxonomy_filtered)==row.names(Top_features))

Top_features_plot<- cbind(Top_features, ASV_16S_Taxonomy_filtered)
...
plot_pallette<- c(brewer.pal(9,'Set3'))

plot_pallette<-c(plot_pallette[1:7],plot_pallette[9])
...
Top_features_abundance<-All_16S[match(row.names(Top_features_plot), row.names(All_16S)),]

print(row.names(ASV_16S_Taxonomy_filtered)==row.names(Top_features_plot))
